...
const TinaPlacePicker = wrapFieldsWithMeta((props: CustomTinaFieldProps) => {
  ...
  const fetchPlace = async (placeUuid: string) => {
    const placeData = await fetch(`https://app.coredata.cloud/core_data/public/v1/places/${placeUuid}?project_ids[]=2`).then(response => response.json());
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await fetch('https://app.coredata.cloud/core_data/public/v1/places?project_ids[]=2').then(response => response.json())
        .then(async response => {
          console.log('hi', response);
          const results = response.list.count;
          const fullData = await fetch(`https://app.coredata.cloud/core_data/public/v1/places?project_ids[]=2&per_page=${results}`).then(response => response.json()).then(response => JSON.stringify(response));
          return fullData});
